@php
    ...
    $scriptUrl = $blueprint->dbGet('{identifier}', 'scriptUrl');
@endphp
...
@if ($provider && $provider !== 'disabled' && $widgetId && !empty(trim($widgetId)))
    @switch($provider)
        ...
        @case('livechat')
            ...
            <noscript><a href="https://www.livechat.com/chat-with/{{ $widgetId }}/" rel="nofollow">Chat with us</a>, powered
                by <a href="https://www.livechat.com/?welcome" rel="noopener nofollow" target="_blank">LiveChat</a></noscript>
        @break
        ...
        @case('liveagent')
            @if ($scriptUrl)
                <script type="text/javascript">
                    ...
                    })(document, '{{ $scriptUrl }}', function(e) {
                        LiveAgent.createButton('{{ $widgetId }}', e);
                    });
                </script>
            @endif
        @break
